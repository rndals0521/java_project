...
public class Ex01 {
	//클래스
	//정의 : 객체(오브젝트, 인스턴스)를 만들기 위한 모든 정보를 가지고 있는 것.(설게도면, 거푸집등으로 표현한다.)
	//       파일형태로 되어있으면 **클래스를 객체로 만들어야만 사용가능하다.
    ...
    //생성자 클래스이름()
	//생성자의 목적은 멤버필드(=데이터) 의 초기화가 목적이다.
	
	//클래스의 구조 : 헤더, 멤버필드, 멤버메소드, 생성자
	//1. 헤더 : [접근제한자][클래스종류] class 클래스이름
	//  -접근제한자 : 외부에서 접근할 수 있는 권한을 말한다. 
	//				  클래스,멤버필드,멤버메소드, 생성자 모두 맨 앞에 표시가능(생략가능)
	//   1)public    : 누구나 다 접근 가능
	//   2)protected : 같은 패키지에 있거나, 상속관계에서는 접근 가능
	//   3)생략      : 같은 패키지에서만 접근 가능
	//   4)private   : 외부에서는 무조건 접근 불가, 내부멤버(필드,메소드)끼리만 접근가능
	//
	// -클래스 종류 : 일반적인 클래스들은 이 부분을 생략
	//				: 특정 클래스는 해당 예약어를 사용 (final, abstract)
	// -class : 클래스임을 나타내는 예약어
	// -클래스 이름 : 클래스이름 = 저장이름 = 생성자
	//				  첫글자 대문자 나머지 소문자, 두단어 이상일때 단어들의 첫글자는 대문자
	//				  특수문자 사용불가, 숫자는 중간이나 끝에 사용가능  ex) JavaTest01, Button01Test
	
	//2. 멤버필드(변수와 상수) : 상태값, 데이터, 속성, 특징
	// -변수 : 언제든지 변할 수 있는 데이터를 저장하는 공간. 첫글자 소문자, 두 단어 이상일때 단어들의 첫글자는 대문자
	//         특수문자 사용불가, 숫자는 중간이나 끝에 사용가능 ex) javaTest01,button01Test;
	// -상수 : 한 번 지정하면 변경할 수 없는 데이터를 저장하는 공간. 
	//  	   모든글자가 대문자, 단어와 단어사이에 _(언더스코어)를 사용할 수 있다.
	// 		   ex) JAVA_TEST_01, BUTTON_01_TEST
	...
	//  1) 접근제한자  : 클래스 접근제한자와 같다. public > protected > 생략 > private
	...
	//                   만약에 실행한 결과를 가지고 가지 않을 때는 반환형에 void라고 쓴다.
	//                   ex) public static void main(String[] args){}
	//                       누구나 다 접근할 수 있고, static메소드이며, 되돌아갈때 무언가를 가지고가지 않는다.
	//  4) 인자=매개변수 : 메소드가 동작을 할 때 필요한 정보를 외부에서 받아서 사용할 때 인자가 필요한다.
	//            오버로딩 : 같은 클래스 안에서 메소드이름은 같고, 
	//								인자의 갯수나 인자의 자료형이 다른 메소드를 말한다.
	//  5) getter() / setter()
	//      getter() : 호출하는 입장에서 메소드를 이용해서 멤버필드의 데이터를 얻어낼 수 있다.
	//      setter() : 호출하는 입장에서 메소드를 이용해서 멤벞필드의 데이터를 설정할 수 있다.
	...
	//       클래스    참조변수   =   객체생성하자           생성자 ;
	//  new하면 인스턴스 : 메모리안을 사용하기 위해서 일정한 공간을 만들어서 사용하는 것.
	//       참조변수를 이용해서 클래스가 가지고 있는 멤버필드나 멤버메소드를 사용할 수 있다.
	//       객체를 만들기 위해서는 생성자를 호출해야하지만
	...
	//     - 만들어진 객체안에 존재하는 멤버필드나 멤버 메소드를 사용하는 방법
	//       참조변수.멤버필드, 참조변수.멤버메소드
	//       예) Scanner scan = new Scanner(System.in);
	//            이때 scan는 참조변수이다.  scan.next(), scan.nextInt();
	//     - 생성자도 오버로딩이 가능하다.
