...
public class Ex04 {
	...
	// 스트림의 종류 : 바이트 스트림, 문자스트림, 바이트-문자 결합 스트림 , 오브젝트스트림
	// 1. 바이트 스트림 (기계중심) : 모든 처리를 1byte 씩 처리
	//    대상 : 1byte로 이루어진 파일들(영상, 소리, 사진 등 모든 파일들을 말한다.
	//           영문자 대소문자, 숫자를 스트림할 수 있다. (영어이외의 문자는 안됨)
	...
	// 3. 바이트-문자 결합스트림 : 기계를 통해 입/출력한 정보를 사람이 사용할 수 있도록 입/출력
	//    해당 클래스 : inputStramReader(입력) , OutputStreamWriter(출력)
	
	// 4. 오브젝트 스트림 : 객체를 직렬화 한 후 객체 단위로 입/출력하낟.
	//                      (객체 직렬화)
